...
> **Дисклеймер**: Из-за политики HF к скачиванию моделей без ввода токена
> скорость скачивания эмбеддинг-модели `multilingual-e5-base` может быть ограниченной. В таком случае необходимо
> в `.env` внести токен (см `.env.example`) и запустить скрипт `scrpits/download_embedding_model.py` по скачиванию
> эмбеддинг-модели, но можно и просто ввести токен и воспользоваться `.sh` скриптами ниже.
> Использование python скрипта лишь незначительно облегчит процесс, потому что модель будет скачиваться вне развертывания docker контейнеров.
