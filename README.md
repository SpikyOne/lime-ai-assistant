...
# 7. Что происходит при старте Ollama
...
Это осознанный компромисс в пользу стабильности и умеренного потребления памяти.
...
Поле `sources` является дополнительным преимуществом относительно минимального формата ТЗ и позволяет показать пользователю первоисточник ответа.

---

# 10. Диагностика Docker
...
# 11. Диагностика в нескольких консолях
...
# 14. Когда нужно пересобирать систему
...
# 16. Локальный запуск без Docker
...
# 19. Ограничение предметной области
...
# 20. Защита внутренней реализации
...
# 25. Место для скриншотов

Ниже рекомендуется разместить три основных скриншота демонстрации:

| Запущенная система | Работа AI-ассистента | Swagger / API |
|---|---|---|
| `![Скриншот 1](docs/images/app-running.png)` | `![Скриншот 2](docs/images/chat-answer.png)` | `![Скриншот 3](docs/images/swagger.png)` |
| Главная страница / demo | Пример вопроса и ответа | Документация API и `/chat` |

Дополнительно можно хранить изображения в:
...
Перед финальной приёмкой рекомендуется добавить отдельный evaluation-набор:
...
\* Эти требования реализованы прежде всего через архитектуру, guardrails и системные инструкции. Для приёмки их необходимо подтвердить отдельной тестовой выборкой.
...
При этом для формальной приёмки по ТЗ необходимо отдельно подтвердить экспериментально два ключевых показателя: **≥80% корректных ответов на выборке из 20 вопросов** и **производительность/конкурентность на целевом железе**.
